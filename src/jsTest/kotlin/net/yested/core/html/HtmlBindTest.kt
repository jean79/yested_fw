...
class HtmlBindTest {
    ...
    @Test
    fun repeatLiveShouldReflectData() {
        ...
        validateDataChanges(ul!!, data, suffix = "Last", animate = false)
    }

    @Test
    fun tableShouldReflectData() {
        tableShouldReflectData(animate = false)
    }

    @Test
    fun tableShouldReflectData_animated() {
        tableShouldReflectData(animate = true)
    }

    private fun tableShouldReflectData(animate: Boolean) {
        val data: Property<List<Int>?> = listOf(1).toProperty()
        ...
        validateDataChanges(tbody!!, data, animate = animate)
    }

    private fun validateDataChanges(containerElement: HTMLElement, data: Property<List<Int>?>, suffix: String = "", animate: Boolean) {
        ...
        validateListAsserts(listAssertIterator, containerElement, data, if (animate) 500 else 0)
    }

    private fun validateListAsserts(listAssertIterator: Iterator<ListAssert>, containerElement: HTMLElement?, data: Property<List<Int>?>, stepDelay: Int) {
        if (listAssertIterator.hasNext()) {
            ...
            if (data.get() != listAssert.list) {
                ...
                window.setTimeout({
                    ...
                    validateListAsserts(listAssertIterator, containerElement, data, stepDelay)
                }, stepDelay)
            } else {
